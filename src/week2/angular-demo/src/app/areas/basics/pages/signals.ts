...
@Component({
  ...
  template: `
    <div class="flex flex-row p-8 gap-4" [class.animate-pulse]="store.magic()">
      <button class="btn btn-sm btn-warning btn-circle" (click)="store.decrement()">-</button>
      <span class="mx-2">{{ store.current() }}</span>
      <button class="btn btn-sm btn-primary btn-circle" (click)="store.increment()">+</button>
      <button
